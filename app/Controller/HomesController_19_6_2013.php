...
class HomesController extends AppController {
	...
	public function index($leadId=null){
		...
		$id='';	
		$number='';	
		(isset($this->request->query['number'])) ? $number=$this->request->query['number'] : $number='';
		(isset($this->request->query['Country'])) ? $country=$this->request->query['Country'] : $country='';
		(isset($this->request->query['id'])) ? $id=$this->request->query['id'] : $id='';
		(isset($this->request->query['session_id'])) ? $five9sessionid=$this->request->query['session_id'] : $five9sessionid='';
		(isset($this->request->query['full_name'])) ? $agent_name=$this->request->query['full_name'] : $agent_name='';
		...
		if(empty($id)) {
			if(isset($this->request->query['campaign_id'])) {
				...
				$campagin_data=$this->Campaign->find('first',array('conditions'=>array('Campaign.five9_campaign_id'=>$this->request->query['campaign_id']),'fields'=>'Campaign.id,Campaign.customer_id'));
				
				
				
					$checkLead=$this->Lead->find('count',array('conditions'=>array('Lead.phone'=>$number)));
					if($checkLead){
						$leadRec=$this->Lead->find('first',array('conditions'=>array('Lead.phone'=>$number),'fields'=>'Lead.id'));
						...
						$this->Lead->create();
						$data['Lead']['phone']=$number;
						$data['Lead']['campaign_id']=$campagin_data['Campaign']['id'];
						$data['Lead']['customer_id']=$campagin_data['Campaign']['customer_id'];
		...
		if(isset($this->request->query['disposition_id']) and $this->request->query['disposition_id']==-1){
			...
			(isset($this->request->query['id'])) ? $data['CallData']['lead_id']=$this->request->query['id'] : $data['CallData']['lead_id']=$id;

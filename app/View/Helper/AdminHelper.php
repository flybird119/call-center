...
class AdminHelper extends Helper {
	...
	*/	
	...
	public function CreateNewLink ($extra_path = null)
	{
		...
		$path =  '/' . $this->params['controller'] . '/new_content/' . $extra_path;

		if($this->params['plugin'] != "")
			$path = '/' . $this->params['plugin'] . $path;
			
		return($this->Html->link('<span>'.$this->Html->image('content/add.png', array('width' => '12', 'height' => '12', 'alt' => '')).'&nbsp;' .$title.'</span>', $path, array('escape' => false, 'class' => 'button-text')));
	}
	...
	public function ActionBar($options = null, $new = true, $extra_path = null, $export = false, $import = false) 
	{
		...
		if(!empty($options))
		{
                    /*Added for content Page by Kapil ==> style="width:15%;float:right;" */
				$content .= '<select name="multiaction" style="width:15%;float:right;" onchange="if (this.form.onsubmit == undefined) { this.form.submit(); } else { if (this.form.onsubmit() != false) { this.form.submit();} return false;} ">';
	...
	public function MoveButtons ($data, $count) 
	{
		...
		if($data['order'] < $count)
		{
			$button .= $this->Ajax->link($this->Html->image('content/down.png', array('alt' => __('Down', true))),'null', $options = array('escape' => false, 'url' => 'admin_move/' . $data['id'] . '/down', 'update' => 'content'),null,false);		
		}
		else
		{
			$button .= $this->Html->link($this->Html->image('transparency.png', array('width' => '16')), 'javascript:void(0);', array('escape' => false));
		}
		if($data['order'] > 1)
		{
			$button .= $this->Ajax->link($this->Html->image('content/up.png', array('alt' => __('Up', true))),'null', $options = array('escape' => false, 'url' => 'admin_move/' . $data['id'] . '/up', 'update' => 'content'),null,false);		
		}
		return($button);
	}
	...
	public function DefaultButton ($data)
	{
		if($data['default'] == 1)
		{
			$button = $this->Html->image('content/true.png', array('alt' => __('True', true)));
		}
		else
		{
			$button = $this->Ajax->link($this->Html->image('content/false.png', array('alt' => __('False', true))),'null', $options = array('escape' => false, 'url' => 'admin_set_as_default/' . $data['id'], 'update' => 'content'),null,false);		
		}
		return($button);
	}
	...
	{
		if($action == 'delete' || $action == 'trashcan')
			$confirm_text = __('Confirm delete action?',true);
		else
			$confirm_text = null;
			
		$button = $this->Html->link('<span class="'.$action.'-10 plix-10"></span>', $path, array('escape' => false,'class'=>'button-icon tip-s'), $confirm_text);
		return($button);		
	}
	...
	*/	
	public function linkButton ($title, $url, $icon = null, $parameters = null, $confirmMessage = false)
	{
		...
		if (!empty($icon) && file_exists(IMAGES . '/content/' . $icon)) {
			$content = $this->Html->link('<span>'.$this->Html->image('/content/'.$icon, array('width' => '12', 'height' => '12', 'alt' => '')).'&nbsp;' .__($title,true).'</span>', $url, $parameters, $confirmMessage);
		} else {
